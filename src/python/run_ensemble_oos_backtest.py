"""
Out-of-Sample Backtest for Ensemble Strategy

Tests the ensemble strategy (vol breakout + sentiment) on 2020-2022 data.
Development was done on 2023-2024 data.

This validates whether the sentiment enhancement provides real value OOS.
...
def run_ensemble_oos_backtest(method: str = 'either'):
    """Run OOS backtest of ensemble strategy."""
    print("=" * 80)
    print(" ENSEMBLE OOS BACKTEST (2020-2022)")
    ...
    print("\n" + "=" * 80)
    print(" ENSEMBLE OOS BACKTEST RESULTS (2020-2022)")
    print("=" * 80)
    ...
    print(f"\n--- Comparison to Baseline OOS ---")
    print(f"  Vol Breakout OOS:   +$496,380 (validated)")
    print(f"  Ensemble OOS:       ${results.get('net_pnl', 0):,.2f}")

    improvement = results.get('net_pnl', 0) - 496380
    print(f"  Difference:         ${improvement:,.2f} ({improvement/496380*100:.1f}%)")
    ...
    trades_df.to_csv(output_dir / f'ensemble_oos_trades_{timestamp}.csv', index=False)
    logger.info(f"\nTrades saved to: ensemble_oos_trades_{timestamp}.csv")

    return results, all_metrics


if __name__ == "__main__":
    run_ensemble_oos_backtest(method='either')
